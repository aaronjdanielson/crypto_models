...
def custom_collate_fn(batch: List[Dict]) -> Dict[str, torch.Tensor]:
    """
    Custom collate function to handle batching for different dataset splits.
    - In "forecast" mode, there are no targets.
    """
    ...
    # Only add targets if available (not in forecast mode)
...
if __name__ == "__main__":
    ...
    ### **Forecast Loader Test**
    forecast_dataset = CryptoDataset(
        file_path='data/combined_data.csv',
        lookback=28,
        horizon=14,
        date_attrs=['day_of_week', 'day_of_month', 'month', 'quarter', 'year_since_earliest_date'],
    ...
    forecast_loader = DataLoader(forecast_dataset, batch_size=batch_size, shuffle=False, collate_fn=custom_collate_fn)

    print("\n==== FORECAST LOADER TEST ====")
    for batch in forecast_loader:
        print(f"Forecast Historical Data Shape: {batch['historical_data'].shape}")
        print(f"Forecast Historical Mask Shape: {batch['historical_mask'].shape}")
        print(f"Forecast Known Features Shape: {batch['date_features'].shape}")
        print(f"Forecast Historical Date Ranges: {batch['historical_date_ranges']}")

        # Ensure 'targets' are not in forecast mode
        assert 'targets' not in batch, "Forecast mode should not include targets!"
        break  # Test first batch only
