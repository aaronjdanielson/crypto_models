...
#         target_date_ranges (numpy array, optional): A single 1D array of numpy.datetime64 for the x-axis.
#         crypto_to_index (dict, optional): Mapping from crypto names to channel indices.
#         target_scaler (CryptoScaler, optional): Scaler object to inverse transform the targets and predictions.
#         forecast_mode (bool): Set to True for forecasting without targets.
...
#     # Reverse crypto_to_index mapping for lookup
#     index_to_crypto = {v: k for k, v in crypto_to_index.items()} if crypto_to_index else {}
...
# def plot_predictions(predictions, targets=None, quantiles=None, target_date_ranges=None, crypto_to_index = None, forecast_mode=False):
#     """
#     Plot the predictions against the true targets for each crypto with date labels.
...
#     # Ensure target_date_ranges is properly formatted
#     if target_date_ranges is not None:
#         date_range = np.array(target_date_ranges).flatten().astype('datetime64[D]').astype(str)  # Ensure 1D format
#     else:
#         date_range = np.arange(predictions.size(2))  # Default to sequential indices if no dates provided
...
# def plot_predictions(predictions, targets=None, quantiles=None, target_date_ranges=None, forecast_mode=False):
#     """
#     Plot the predictions against the true targets for each crypto with date labels.
...
#         targets (torch.Tensor, optional): Ground truth targets of shape [num_samples, num_channels, tau].
#         quantiles (list, optional): List of quantile levels corresponding to the prediction intervals.
#         target_date_ranges (list or numpy array, optional): A single array of numpy.datetime64 for the x-axis.
#         forecast_mode (bool): Set to True for forecasting without targets.
...
#     # Ensure target_date_ranges is formatted correctly
#     if target_date_ranges is not None:
#         date_range = np.array(target_date_ranges).astype('datetime64[D]').astype(str)  # Convert to date-only format
#     else:
#         date_range = np.arange(predictions.size(2))  # Default to sequential indices if no dates provided
...
#         if not forecast_mode and targets is not None:
#             plt.plot(date_range, targets[:, channel, :].cpu().numpy().flatten(), label="True Values", color="blue")

#         for i, q in enumerate(quantiles):
#             plt.plot(date_range, predictions[:, channel, :, i].cpu().numpy().flatten(), label=f"Quantile {q:.2f}", linestyle="--")
...
#         targets (torch.Tensor, optional): Ground truth targets of shape [num_samples, num_channels, tau].
#         quantiles (list, optional): List of quantile levels corresponding to the prediction intervals.
#         target_date_ranges (list of lists, optional): List containing arrays of numpy.datetime64 for time axis.
#         forecast_mode (bool): Set to True for forecasting without targets.
...
#         if target_date_ranges is not None:
#             date_range = np.array(target_date_ranges[channel]).astype(str)  # Convert to string to remove time portion
#         else:
#             date_range = np.arange(predictions.size(2))  # Default to indices if no dates provided
...
#         if not forecast_mode and targets is not None:
#             plt.plot(targets[:, channel, :].cpu().numpy().flatten(), label="True Values", color="blue")  # Move to CPU before converting
...
#         for i, q in enumerate(quantiles):
#             print()
#             plt.plot(predictions[:, channel, :, i].cpu().numpy().flatten(), label=f"Quantile {q:.2f}", linestyle="--")  # Move to CPU before converting
